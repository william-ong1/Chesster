"""
Given a board object, extract the features of the board state
and return them as a tensor.
The features right now are:
...
class FeatureExtraction:
    """Used to extract features from a game board."""
    ...
    def extract_features(self) -> torch.Tensor:
        ...
        queen_threatened_white = self.is_threatened(
            chess.PieceType.QUEEN, chess.WHITE
        )
        queen_threatened_black = self.is_threatened(
            chess.PieceType.QUEEN, chess.BLACK
        )
        rook_threatened_white = self.is_threatened(
            chess.PieceType.ROOK, chess.WHITE
        )
        rook_threatened_black = self.is_threatened(
            chess.PieceType.ROOK, chess.BLACK
        )
        bishop_threatened_white = self.is_threatened(
            chess.PieceType.BISHOP, chess.WHITE
        )
        bishop_threatened_black = self.is_threatened(
            chess.PieceType.BISHOP, chess.BLACK
        )
        knight_threatened_white = self.is_threatened(
            chess.PieceType.KNIGHT, chess.WHITE
        )
        knight_threatened_black = self.is_threatened(
            chess.PieceType.KNIGHT, chess.BLACK
        )
        pawn_threatened_white = self.is_threatened(
            chess.PieceType.PAWN, chess.WHITE
        )
        pawn_threatened_black = self.is_threatened(
            chess.PieceType.PAWN, chess.BLACK
        )
    ...
    def _squares_controlled_count(self, color: chess.Color) -> int:
        ...
        """
        return sum(
            1 for sq in chess.SQUARES if self.board.is_attacked_by(color, sq)
        )
    ...
    def is_threatened(self, piece: chess.PieceType, color: chess.Color) -> bool:
        ...
        for square in chess.SQUARES:
            if (
                self.board.piece_at(square) is not None
                and self.board.piece_at(square).color == color
                and self.board.piece_at(square).piece_type == piece
            ):
        ...
        return torch.tensor(
            [
                (
                    ...
                    and self.board.piece_at(square).color == color
                    and self.board.piece_at(square).piece_type
