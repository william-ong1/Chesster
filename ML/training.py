...
# Lebam Amare, Raghav Ramesh, and Danyuan Wang

"""
Code for training the model from a given architecture, using supervised
learning and a weighted sum of the player's data, and the evaluation of
a preexisting chess engine (Stockfish). Pull data from MongoDB and use it
to train the model. After training, save the model to MongoDB.
"""
...
from ML.model_architectures import (
    ThreeLayerNN,
)  # Import the 3 layer neural network architecture
import uuid
...
class Training:
    ...
    def __init__(
        ...
        self.optimizer = optim_class(
            self.model.parameters(), defaults={"lr": self.learning_rate}
        )
    ...
    def _load_architecture(self, architecture: str) -> nn.Module:
        ...
        if architecture == "3_layer_nn":
            return (
                ThreeLayerNN()
            )  # TODO: figure out function call and temp. params
        else:
            raise ValueError(
